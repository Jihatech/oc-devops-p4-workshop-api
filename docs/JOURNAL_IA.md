...
| 2026-07-14 | Revue mentor : mots de passe de démo en clair dans des `values*.yaml` publics. | Choix **assumé** (valeurs jetables, Minikube local, aucun service exposé). | Ajout d'une section « Sécurité des secrets » (`docs/DEPLOIEMENT_K8S.md`) + commentaires dans les values : en prod, `--set db.password=...` / **SOPS** / **External Secrets**. Vérifié que le chart accepte l'override (`--set` → secret régénéré). Le template ne change pas : seule l'**origine** de la valeur change. |
